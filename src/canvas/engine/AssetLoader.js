...
const { AssetError } = require('../../errors/ModularError');
...
class AssetLoader {
   ...
   * @param {number} [options.errorTtl=30000] - Error cache TTL in ms (default 30s)
   */
  constructor(cache, options = {}) {
    this.cache = cache;
    this.loadingPromises = new Map();
    this.errors = new Map();
    this.errorTtl = options.errorTtl || 30000;
  }
  ...
  async loadImage(url) {
    ...
    // Check if previously failed (with TTL)
    if (this.errors.has(url)) {
      const errorEntry = this.errors.get(url);
      if (Date.now() - errorEntry.timestamp < this.errorTtl) {
        throw errorEntry.error;
      }
    ...
    try {
      ...
      this.errors.set(url, { error: err, timestamp: Date.now() });
      throw err;
    } finally {
  ...
  async _loadImageInternal(url) {
    try {
      return await getLoadImage()(url);
    } catch (err) {
      throw new AssetError(`Failed to load image: ${url}`, { url, originalError: err.message });
    }
  }
